...
def main():
    ...
    ap.add_argument("--doc", default=_latest_doc("框架_独立审核包_v"))
    ...
    if not os.path.exists(rp):
        ...
        if os.path.exists(lp_):
            ...
            ln_ = len(LL.get("checks", []))
            allowed.add(f"{ln_}/{ln_}")
    ...
    if os.path.exists(lp):
        ...
        add("G-4.1", "layout_report 判定项数", n, ">0", n > 0)
        ...
        import re as _re4
        _h42 = bool(_re4.search(r"排版[^\n]{0,20}" + _re4.escape(f"{npass}/{n}"), s))
        add("G-4.2", "排版声称 N/N 与 JSON 一致(上下文匹配)",
            _h42 or n == 0, f"排版语境含 {npass}/{n}", _h42 or n == 0)
    else:
...
#   —— 因"8 点"不在废弃短语表中。语义一致性需人工复核或另设检查。
    ...
    try:
        ...
        #   "注入违规却抓不到" → 被判空规。
        _rp = (a.doc if "合集" in os.path.basename(a.doc)
                else os.path.join(WS, "四角度审核报告_合集.md"))
        ...
        _ver = (json.load(open(_bvp, encoding="utf-8"))["version"]
                 ...
                 json.load(open(os.path.join(WS, "MANIFEST.json"),
                                encoding="utf-8")).get("version", "0"))
        _src0 = a.src or os.path.join(WS, "框架_独立审核包_v1.md")
        # G-13-EXEMPT: G-10.1 语义即'版本化副本 vs 源文档',
        #   必须有稳定固定路径,不可用 a.doc(否则自比自恒 PASS)。
        _vp = os.path.join(WS, "框架_独立审核包_v%s_20260911.md" % _ver)
        ...
        for _fn in _GUARD_SCRIPTS:
            ...
            for _i, _line in enumerate(_src):
                ...
                add("G-13.%s.%s%d" % (_fn[:4], _hit[0], _i),
                    f"{_fn}:{_i+1} {_hit[0]} 硬编码主文档须有 a.doc 回退",
                    _ok13, "期望=True", _ok13)
        # ★ 覆盖度自检:至少扫到已知若干处(否则说明模式全部失效 = 空规)
        add("G-13.lint", "G-13 至少命中 1 处硬编码模式(防模式失效变空规)",
            _n13, ">=1", _n13 >= 1)
    ...
    try:
        ...
        _apx_p = (a.doc if '附录' in os.path.basename(a.doc)
                  else os.path.join(WS, '附录_原始输出.md'))
        ...
        _fr_p = (a.doc if ('框架' in os.path.basename(a.doc)
                           or '审核包' in os.path.basename(a.doc))
                 else os.path.join(WS, '框架_独立审核包_v1.md'))
        ...
        _p11 = os.path.join(WS, '四角度审核报告_合集.md')
        if '合集' in os.path.basename(a.doc):
            _p11 = a.doc
    ...
    try:
        import re as _re12
        # ⚠ 审计 v2.7：原以 MANIFEST 为基准，但 MANIFEST 在 build 第 12 步
        #   才更新，而本守卫在第 7 步跑 —— 拿滞后的派生物当基准必然 FAIL。
        #   按"数字唯一来源"原则：run_id 的权威源是 report.json，
        #   MANIFEST 只是它的派生副本。故改为对 report.json 比对。
        _rj = json.load(open(os.path.join(ROOT, "results", "report.json"),
                             encoding="utf-8"))
        # ⚠ v2.8:原以 MANIFEST 为基准 → 构建到 v2.8 时 banner 已是 v2.8
        #   而 MANIFEST 仍 v2.7,必然 FAIL(滞后派生物当基准)。
        #   与 run_id 的教训同源 —— 上次我只改了 rid,ver 漏改,同一 bug 修一半。
        #   权威源改为 results/build_version.json(build step 0 写入)。
        _bv_p = os.path.join(ROOT, "results", "build_version.json")
        _ver = str(json.load(open(_bv_p, encoding="utf-8"))["version"]) \
            if os.path.exists(_bv_p) else \
            str(json.load(open(os.path.join(WS, "MANIFEST.json"),
                               encoding="utf-8")).get("version"))
        _rid = str(_rj.get("run_metadata", {}).get("run_id", ""))
        for _tag, _fn in (("框架", "框架_独立审核包"), ("合集", "四角度审核报告_合集")):
            ...
            add("G-12.%s.rid" % _tag, "%s banner run_id == report.json(权威源)" % _tag,
                (_mr.group(1)[:16] if _mr else None), _rid[:16],
                bool(_mr and _rid.startswith(_mr.group(1)[:16])))
            # G-12b:全部 run_id 声明(不只 banner)均须 == report.json
            _bad_rid = [m.group(1)[:16] for m in _mr_all
                        if not _rid.startswith(m.group(1)[:16])]
            add("G-12.%s.rid-all" % _tag,
                "%s 全部 run_id 声明 == report.json(%d 处)" % (_tag, len(_mr_all)),
                ("不一致: " + ", ".join(_bad_rid)) if _bad_rid else "全部一致",
                "全部一致", not _bad_rid)
    ...
    print("\n" + "=" * 74)
    print(f"  一致性守卫: {npass}/{len(rows)} 通过, 失败 {nfail} 项")
    print("=" * 74)
    if nfail:
